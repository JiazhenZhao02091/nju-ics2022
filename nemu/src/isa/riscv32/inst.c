...
/*
    ...
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2));
    // INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I,   R(rd) = SEXT( Mr(src1 + imm, 1), 32));
    ...
    INSTPAT("??????? ????? ????? 101 ????? 00100 11", srli    , I,  R(rd) = src1 >> BITS(imm, 5, 0));
...
INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if ((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm);
INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(rd) = SEXT(src1, 32) / SEXT(src2, 32));
INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; MAYBE_FUNC_JAL(s); R(rd) = s->pc + 4);
